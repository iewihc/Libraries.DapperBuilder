...
namespace Libraries.DapperBuilder
{
    ...
    public class CommandBuilder : ICompleteCommand
    {
        ...
        /// </summary>
        public CommandBuilder AddParameter(string parameterName, object parameterValue = null, DbType? dbType = null,
            ParameterDirection? direction = null, int? size = null, byte? precision = null, byte? scale = null)
        {
            ...
            _parametersStr = string.Join(", ",
                _parameters.ParameterNames.ToList()
                    .Select(n => "@" + n + "='" + Convert.ToString(Parameters.Get<dynamic>(n)) + "'"));
            return this;
        }
        ...
        public CommandBuilder Append(FormattableString statement)
        {
            ...
            if (!string.IsNullOrWhiteSpace(sql))
            {
                ...
                string currentLine = _command.ToString().Split(new[] {"\r\n", "\r", "\n"}, StringSplitOptions.None)
                    .LastOrDefault();
                if (currentLine != null && currentLine.Length > 0 && !char.IsWhiteSpace(currentLine.Last()) &&
                    !char.IsWhiteSpace(sql[0]))
                    _command.Append(" ");
            }
        ...
        protected string AdjustMultilineString(string block)
        {
            ...
            if (
                nonEmptyLines.Count <=
                1) // if there's not at least 2 non-empty lines, assume that we don't need to adjust anything
                return block;
            ...
            int minNumberOfSpaces = nonEmptyLines
                .Select(nonEmptyLine => nonEmptyLine.Length - nonEmptyLine.TrimStart().Length).Min();
        ...
        public virtual string Sql =>
            _command.ToString(); // base CommandBuilder will just have a single variable for the statement;
